...
impl SwapEncoder for BalancerV3SwapEncoder {
    ...
    fn encode_swap(
        ...
        let pool_type = get_static_attribute(swap, "pool_type")?;

        let packed_byte = if pool_type == b"LiquidityBuffer" {
            let tokens = &swap.component.tokens;
            let (is_wrap_in, is_unwrap_out) = match (tokens.get(0), tokens.get(1)) {
                (Some(wrapped_token), Some(underlying_token)) => {
                    let wrap_in =
                        swap.token_in == *underlying_token && swap.token_out == *wrapped_token;
                    let unwrap_out =
                        swap.token_in == *wrapped_token && swap.token_out == *underlying_token;
                    (wrap_in, unwrap_out)
                }
                _ => (false, false),
            };
            // Bit:  |                3 |         2  |       1 |         0 |
            // Field: transfer_type_bits | unwrap_out | wrap_in | is_buffer |
            ((encoding_context.transfer_type as u8) << 3) |
                ((is_unwrap_out as u8) << 2) |
                ((is_wrap_in as u8) << 1) |
                1u8 // is_buffer = true
        } else {
            (encoding_context.transfer_type as u8) << 3
...
mod tests {
    ...
    mod balancer_v3 {
        ...
        fn test_encode_balancer_v3() {
            let balancer_pool = ProtocolComponent {
                ...
                static_attributes: {
                    ...
                    attrs.insert(
                        "pool_type".to_string(),
                        Bytes::from("0x537461626c65506f6f6c466163746f7279"),
                    );
                    attrs
                },
                tokens: vec![
            ...
            assert_eq!(
                ...
                String::from(concat!(
                    ...
                    // is_buffer, wrap_in, unwrap_out: false, TransferType: Transfer
                    "08",
                    // receiver
                    "1d96f2f6bef1202e4ce1ff6dad0c2cb002861d3e",
                ))
        ...
        fn test_encode_balancer_v3_with_wrap_in() {
            let balancer_pool = ProtocolComponent {
                id: String::from("0x1202f5c7b4b9e47a1a484e8b270be34dbbc75055"),
                protocol_system: String::from("vm:balancer_v3"),
                static_attributes: {
                    let mut attrs = HashMap::new();
                    attrs.insert(
                        "pool_type".to_string(),
                        Bytes::from("0x4c6971756964697479427566666572"),
                    );
                    attrs
                },
                tokens: vec![
                    Bytes::from_str("0x1202f5c7b4b9e47a1a484e8b270be34dbbc75055").unwrap(), /* wstUSR */
                    Bytes::from_str("0x66a1e37c9b0eaddca17d3662d6c05f4decf3e110").unwrap(), // USR
                ],
                ..Default::default()
            };
            let token_in = Bytes::from("0x66a1e37c9b0eaddca17d3662d6c05f4decf3e110"); // USR
            let token_out = Bytes::from("0x1202f5c7b4b9e47a1a484e8b270be34dbbc75055"); // wstUSR
            let swap = SwapBuilder::new(balancer_pool, token_in.clone(), token_out.clone()).build();
            let encoding_context = EncodingContext {
                // The receiver was generated with `makeAddr("bob") using forge`
                receiver: Bytes::from("0x1d96f2f6bef1202e4ce1ff6dad0c2cb002861d3e"),
            ...
            assert_eq!(
                ...
                String::from(concat!(
                    // token in
                    "66a1e37c9b0eaddca17d3662d6c05f4decf3e110",
                    // token out
                    "1202f5c7b4b9e47a1a484e8b270be34dbbc75055",
                    // pool id
                    "1202f5c7b4b9e47a1a484e8b270be34dbbc75055",
                    // is_buffer:True, wrap_in:True, unwrap_out: false, TransferType: Transfer
                    "0b",
                    // receiver
                    "1d96f2f6bef1202e4ce1ff6dad0c2cb002861d3e",
                ))
        ...
        fn test_encode_balancer_v3_with_unwrap_out() {
            ...
            let token_in = Bytes::from("0x1202f5c7b4b9e47a1a484e8b270be34dbbc75055"); // wstUSR
            let token_out = Bytes::from("0x66a1e37c9b0eaddca17d3662d6c05f4decf3e110"); // USR
            let swap = SwapBuilder::new(balancer_pool, token_in.clone(), token_out.clone()).build();
            let encoding_context = EncodingContext {
                // The receiver was generated with `makeAddr("bob") using forge`
                receiver: Bytes::from("0x1d96f2f6bef1202e4ce1ff6dad0c2cb002861d3e"),
            ...
            assert_eq!(
                ...
                String::from(concat!(
                    ...
                    // pool id
                    "1202f5c7b4b9e47a1a484e8b270be34dbbc75055",
                    // is_buffer:True, wrap_in:False, unwrap_out: True, TransferType: Transfer
                    "0d",
                    // receiver
                    "1d96f2f6bef1202e4ce1ff6dad0c2cb002861d3e",
                ))
