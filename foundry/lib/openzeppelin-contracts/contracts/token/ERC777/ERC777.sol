...
contract ERC777 is Context, IERC777, IERC20 {
    ...
    constructor(
        ...
        _ERC1820_REGISTRY.setInterfaceImplementer(
            address(this), keccak256("ERC777Token"), address(this)
        );
        _ERC1820_REGISTRY.setInterfaceImplementer(
            address(this), keccak256("ERC20Token"), address(this)
        );
    }
    ...
    function authorizeOperator(address operator) public virtual override {
        require(
            _msgSender() != operator, "ERC777: authorizing self as operator"
        );
    ...
    function _move(
        ...
        uint256 fromBalance = _balances[from];
        require(
            fromBalance >= amount, "ERC777: transfer amount exceeds balance"
        );
        unchecked {
            _balances[from] = fromBalance - amount;
        }
    ...
    function _callTokensToSend(
        ...
        address implementer = _ERC1820_REGISTRY.getInterfaceImplementer(
            from, _TOKENS_SENDER_INTERFACE_HASH
        );
    ...
    function _callTokensReceived(
        ...
        address implementer = _ERC1820_REGISTRY.getInterfaceImplementer(
            to, _TOKENS_RECIPIENT_INTERFACE_HASH
        );
        if (implementer != address(0)) {
        ...
        virtual
    {
        ...
        if (currentAllowance != type(uint256).max) {
            require(
                currentAllowance >= amount, "ERC777: insufficient allowance"
            );
            unchecked {
