...
abstract contract Test is DSTest, Script {
    ...
    function deployCode(string memory what) internal returns (address addr) {
        ...
        require(
            addr != address(0), "Test deployCode(string): Deployment failed."
        );
    }
        ...
        returns (EIP1559ScriptArtifact memory)
    {
        ...
        artifact.transactions =
            rawToConvertedEIPTx1559s(rawArtifact.transactions);
        return artifact;
    }
        ...
        returns (ReceiptLog[] memory)
    {
        ...
        for (uint256 i; i < rawLogs.length; i++) {
            ...
            logs[i].transactionLogIndex =
                bytesToUint(rawLogs[i].transactionLogIndex);
...
library stdStorage {
    ...
    function find(StdStorage storage self) internal returns (uint256) {
        ...
        if (
            ...
            return self.slots[who][fsig][keccak256(
                abi.encodePacked(ins, field_depth)
            )];
        }
        ...
        if (reads.length == 1) {
            ...
            for (uint256 i = 0; i < reads.length; i++) {
                ...
                if (success && fdat == bytes32(hex"1337")) {
                    ...
                    self.slots[who][fsig][keccak256(
                        abi.encodePacked(ins, field_depth)
                    )] = uint256(reads[i]);
                    self.finds[who][fsig][keccak256(
                        abi.encodePacked(ins, field_depth)
                    )] = true;
