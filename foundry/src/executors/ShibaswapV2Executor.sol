...
error ShibaswapV2Executor__InvalidDataLength();
error ShibaswapV2Executor__InvalidFactory();
error ShibaswapV2Executor__InvalidTarget();
error ShibaswapV2Executor__InvalidInitCode();

contract ShibaswapV2Executor is IExecutor, ICallback, RestrictTransferFrom {
    ...
    constructor(
        ...
        if (_initCode == bytes32(0)) {
            revert ShibaswapV2Executor__InvalidInitCode();
        }
        factory = _factory;
        initCode = _initCode;
        self = address(this);
    ...
    {
        if (data.length != 85) {
            revert ShibaswapV2Executor__InvalidDataLength();
        }
    ...
    function _verifyPairAddress(
        ...
        if (pool != target) {
            revert ShibaswapV2Executor__InvalidTarget();
        }
    }
}
