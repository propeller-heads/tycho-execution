...
contract TychoRouterActionType is
    AccessControl,
    Dispatcher,
    Pausable,
    ReentrancyGuard
{
    ...
    /**
     * @dev Returns arguments required to perform a batch execution
     */
    function _decodeBatchExecute(bytes calldata data)
        internal
        pure
        returns (uint256 amount, ActionType actionType, bytes calldata res)
    {
        assembly {
            amount := calldataload(data.offset)
            actionType := shr(248, calldataload(add(data.offset, 0x20)))
            res.offset := add(data.offset, 0x21)
            res.length := sub(data.length, 0x21)
        }
    }
     ...
     */
    function batchExecute(bytes calldata data) public {
        uint256 amount;
        bytes calldata actionData;
        bytes calldata batchElement;
        ActionType actionType;
        while (data.length > 0) {
            console.logString("Fetching next batch element");
            (batchElement, data) = data.next();
            console.logString("Decoding batch element");
            (amount, actionType, actionData) = _decodeBatchExecute(batchElement);
            console.logString("Batch element decoded");

            _executeAction(amount, actionType, actionData);
        }
        console.logString("Data end reached");
    }
    ...
    function _executeAction(
        ...
        if (type_ == ActionType.TRANSFER) {
            console.logString("PERFORMING TRANSFER");
            (address receiver, address token) = _decodeTransfer(actionData);
            ...
            console.logString("Transfer successfully performed");
        } else if (type_ == ActionType.SPLIT) {
            console.logString("PERFORMING SWAP");
            (
                address tokenIn,
                address tokenOut,
            ...
            ) = _decodeSwapInput(actionData);
            calculatedAmount = swap(
                amount,
                tokenIn,
                tokenOut,
                minAmountOut,
    ...
    function _decodeTransfer(bytes calldata data)
        internal
        pure
        returns (address receiver, address tokenAddress)
    {
        receiver = address(bytes20(data[:20]));
        tokenAddress = address(bytes20(data[20:40]));
    }

    function _decodeSwapInput(bytes calldata data)
        internal
        pure
        returns (
            address tokenIn,
        ...
        )
    {
        ...
        minAmountOut = uint256(bytes32(data[40:72]));
        wrapEth = uint8(data[72]) > 0;
        unwrapEth = uint8(data[73]) > 0;
        nTokens = uint256(bytes32(data[74:106]));
        receiver = address(bytes20(data[106:126]));
        swaps = data[126:];
    }
